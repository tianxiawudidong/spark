...
import com.ifchange.spark.algorithms.http.CvTagHttp;
 ...
 * mariaDb
 */
public class FlushCvTagForTobTest {

    private static final Logger LOG = LoggerFactory.getLogger(FlushCvTagForTobTest.class);

    private static final String HOST1 = "192.168.9.13";
    ...
    public static void main(String[] args) {
        ...
        Dataset<String> result = sparkSession.read().textFile(path).repartition(partition)
            .mapPartitions((MapPartitionsFunction<String, String>) iterator -> {
                CvTagHttp.init(5);
                Mysql mysql13 = new Mysql(USERNAME, PASSWORD, "tob_resume_pool_0", HOST1, PORT);
                ...
                while (iterator.hasNext()) {
                    ...
                    if (StringUtils.isNoneBlank(line)) {
                        ...
                        try {
                            ...
                            try {
                                resumeContent =mysql13.getTobResumeDetailContent(sql);
                            } catch (Exception e) {
                                LOG.info("---------------------------------------------------");
                            ...
                            if (StringUtils.isNoneBlank(resumeContent)) {
                                ...
                                if (null != compress) {
                                    String tagPredict = null;
                                    try {
                                        tagPredict = CvAlgorithmsTagUtil.callCvTag(id, compress);
                                    } catch (Exception e) {
                                        LOG.info("id:{} call cv_tag error:{}", id, e.getMessage());
                                    }
                                    if (StringUtils.isNoneBlank(tagPredict)) {
                                        LOG.info("id:{} cv_tag:{}", id, tagPredict);
                                        String value = MyString.toUnicode(tagPredict, false, false);
                                        String encoder = new String(Base64.getEncoder().encode(MyString.gzipCompress(value, "UTF-8")));
                                        String param = "'" + "cv_tag" + "'" + "," + "'" + encoder + "'";
                                        ...
                                        try {
                                            ...
                                            LOG.info(sql2);
                                            int flag = mysql13.executeUpdate(sql2);
                                            LOG.info("id:{} update cv_tag flag:{}", id, flag);
                                            if (flag != 0) {
                                        ...
                                        if (isUpdate == 1) {
                                            try {
                                                ...
                                                LOG.info(sql3);
                                                int flag2 = mysql13.executeUpdate(sql3);
                                        ...
                                        LOG.info("id:{} cv_tag result is empty", id);
