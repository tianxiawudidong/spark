...
public class FlushCvFeatureForIcdc {

    private static final Logger LOG = LoggerFactory.getLogger(FlushCvFeatureForIcdc.class);
    ...
    public static void main(String[] args) {
                        ...
                        try {
                            ...
                            if (StringUtils.isNoneBlank(compressStr) && !StringUtils.equals("null", compressStr)) {
                                ...
                                if (null != compress) {
                                    ...
                                    try {
                                        cvFeature = CvAlgorithmsTagUtil.callCvFeature(id, compress);
                                        LOG.info("id:{} cv_feature:{}", id, cvFeature);
                                    } catch (Exception e) {
                                        LOG.info("id:{} call cv_feature error:{}", id, e.getMessage());
                                    }
                                    if (StringUtils.isNoneBlank(cvFeature)) {
                                        cvFeature = MyString.toUnicode(cvFeature, false);
                                        cvFeature = cvFeature.replace("'", "\\'");
                                        String param = "'" + "cv_feature" + "'" + "," + "'" + cvFeature + "'";
                                        ...
                                        try {
                                            ...
                                            LOG.info("id:{} update cv_feature flag:{}", id, flag);
                                            if (flag != 0) {
        ...
        LOG.info("resume no cv_feature result accumulator value:{}", resumeWithNoWorkAccumulator.value());
