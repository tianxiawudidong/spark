...
import com.ifchange.spark.algorithms.http.CvTitleHttp;
...
public class FlushCvTitleForIcdc {

    private static final Logger LOG = LoggerFactory.getLogger(FlushCvTitleForIcdc.class);
    ...
    public static void main(String[] args) {
                        ...
                        try {
                            ...
                            if (StringUtils.isNoneBlank(compressStr) && !StringUtils.equals("null", compressStr)) {
                                ...
                                if (null != compress) {
                                    String cvTitle = null;
                                    try {
                                        cvTitle = CvAlgorithmsTagUtil.callCvTitle(id, compress);
                                        LOG.info("id:{} cv_title:{}", id, cvTitle);
                                    } catch (Exception e) {
                                        LOG.info("id:{} call cv_title error:{}", id, e.getMessage());
                                    }
                                    if (StringUtils.isNoneBlank(cvTitle)) {
                                        cvTitle = MyString.toUnicode(cvTitle, false);
                                        cvTitle = cvTitle.replace("'", "\\'");
                                        String param = "'" + "cv_title" + "'" + "," + "'" + cvTitle + "'";
                                        ...
                                        try {
                                            ...
                                            LOG.info("id:{} update cv_title flag:{}", id, flag);
                                            if (flag != 0) {
                                        ...
                                        LOG.info("id:{} cv_title result is empty", id);
        ...
        LOG.info("resume no cv_title result accumulator value:{}", resumeWithNoWorkAccumulator.value());
