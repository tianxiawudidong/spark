...
import com.ifchange.spark.algorithms.http.CvDomainHttp;
 ...
 * cv_domain
 */
public class FlushCvDomainForIcdc {

    private static final Logger LOG = LoggerFactory.getLogger(FlushCvDomainForIcdc.class);
                        ...
                        try {
                            ...
                            if (StringUtils.isNoneBlank(compressStr) && !StringUtils.equals("null", compressStr)) {
                                ...
                                if (null != compress) {
                                    ...
                                    String cvDomain = null;
                                    try {
                                        cvDomain = CvAlgorithmsTagUtil.callCvDomain(id, compress, functionMap);
                                        LOG.info("id:{} cv_domain:{}", id, cvDomain);
                                    } catch (Exception e) {
                                        LOG.info("id:{} call cv_domain error:{}", id, e.getMessage());
                                    }
                                    if (StringUtils.isNoneBlank(cvDomain)) {
                                        String value = MyString.toUnicode(cvDomain, false);
                                        value = value.replace("'", "\\'");
                                        String param = "'" + "cv_domain" + "'" + "," + "'" + value + "'";
                                        ...
                                        try {
                                            ...
                                            LOG.info("id:{} update cv_domain flag:{}", id, flag);
                                            if (flag != 0) {
                                        ...
                                        LOG.info("id:{} cv_domain result is empty", id);
