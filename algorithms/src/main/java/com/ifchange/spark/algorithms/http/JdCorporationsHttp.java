 ...
 * @author root
 */
public class JdCorporationsHttp extends AlgorithmsHttp implements Callable<String> {

    private List<Map<String, Object>> workList = new ArrayList<>();

    private static final Logger logger = LoggerFactory.getLogger(JdCorporationsHttp.class);
     ...
     */
    public JdCorporationsHttp(String positionId, List<String> corporations) throws Exception {
        super();
        ...
        if (null != corporations && corporations.size() > 0) {
            for (int i = 0; i < corporations.size(); i++) {
                ...
                map.put("work_id", String.valueOf(i));
                map.put("company_name", corporations.get(i));
                workList.add(map);
            }
        }
    ...
    public String start(JdCorporationsHttp jdCorporationsHttp) throws Exception {
        String result = "";
        Future<String> submit = threadPoolExecutor.submit(jdCorporationsHttp);
