...
import com.ifchange.spark.algorithms.http.CvEducationHttp;
...
public class FlushCvEducationForIcdc {

    private static final Logger LOG = LoggerFactory.getLogger(FlushCvEducationForIcdc.class);
    ...
    public static void main(String[] args) {
                    ...
                    if (StringUtils.isNoneBlank(line)) {
                        ...
                        try {
                            ...
                            if (StringUtils.isNoneBlank(compressStr) && !StringUtils.equals("null", compressStr)) {
                                ...
                                if (null != compress) {
                                    String cvEducation = null;
                                    try {
                                        cvEducation = CvAlgorithmsTagUtil.callCvEducation(id, compress);
                                        LOG.info("id:{} cv_education:{}", id, cvEducation);
                                    } catch (Exception e) {
                                        LOG.info("id:{} call cv_education error:{}", id, e.getMessage());
                                    }
                                    if (StringUtils.isNoneBlank(cvEducation)) {
                                        cvEducation = MyString.toUnicode(cvEducation, false);
                                        cvEducation = cvEducation.replace("'", "\\'");
                                        String param = "'" + "cv_education" + "'" + "," + "'" + cvEducation + "'";
                                        ...
                                        try {
                                            ...
                                            LOG.info("id:{} update cv_education flag:{}", id, flag);
                                            if (flag != 0) {
                                        ...
                                        LOG.info("id:{} cv_education result is empty", id);
        ...
        LOG.info("resume no cv_education result accumulator value:{}", resumeWithNoWorkAccumulator.value());
