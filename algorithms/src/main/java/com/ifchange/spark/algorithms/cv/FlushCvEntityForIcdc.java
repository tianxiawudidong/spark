...
import com.ifchange.spark.algorithms.http.CvEntityHttp;
import com.ifchange.spark.algorithms.http.CvFeatureHttp;
...
public class FlushCvEntityForIcdc {

    private static final Logger LOG = LoggerFactory.getLogger(FlushCvEntityForIcdc.class);
    ...
    public static void main(String[] args) {
                    ...
                    if (StringUtils.isNoneBlank(line)) {
                        ...
                        try {
                            ...
                            if (StringUtils.isNoneBlank(compressStr) && !StringUtils.equals("null", compressStr)) {
                                ...
                                if (null != compress) {
                                    String cvEntity = null;
                                    try {
                                        cvEntity = CvAlgorithmsTagUtil.callCvEntity(id, compress);
                                        LOG.info("id:{} cv_entity:{}", id, cvEntity);
                                    } catch (Exception e) {
                                        LOG.info("id:{} call cv_entity error:{}", id, e.getMessage());
                                    }
                                    if (StringUtils.isNoneBlank(cvEntity)) {
                                        cvEntity = MyString.toUnicode(cvEntity, false);
                                        cvEntity = cvEntity.replace("'", "\\'");
                                        String param = "'" + "cv_entity" + "'" + "," + "'" + cvEntity + "'";
                                        ...
                                        try {
                                            ...
                                            LOG.info("id:{} update cv_entity flag:{}", id, flag);
                                            if (flag != 0) {
                                        ...
                                        LOG.info("id:{} cv_entity result is empty", id);
        ...
        LOG.info("resume no cv_entity result accumulator value:{}", resumeWithNoWorkAccumulator.value());
