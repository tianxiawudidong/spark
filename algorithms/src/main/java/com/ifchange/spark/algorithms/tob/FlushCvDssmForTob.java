...
import com.ifchange.spark.algorithms.http.CvDssmHttp;
...
public class FlushCvDssmForTob {

    private static final Logger LOG = LoggerFactory.getLogger(FlushCvDssmForTob.class);
                        ...
                        try {
                            ...
                            if (StringUtils.isNoneBlank(resumeContent)) {
                                ...
                                if (null != compress) {
                                    String cvDssm = null;
                                    try {
                                        cvDssm = CvAlgorithmsTagUtil.callCvDssm(id, compress);
                                    } catch (Exception e) {
                                        LOG.info("id:{} call cv_dssm error:{}", id, e.getMessage());
                                    }
                                    if (StringUtils.isNoneBlank(cvDssm)) {
                                        JSONObject json = JSONObject.parseObject(cvDssm);
                                        String jsonStr=json.toString();
//                                        LOG.info("id:{} cv_dssm:{}", id, jsonStr);
                                        String value = MyString.toUnicode(jsonStr, false, false);
                                        String encoder = new String(Base64.getEncoder().encode(MyString.gzipCompress(value, "UTF-8")));
                                        String param = "'" + "cv_dssm" + "'" + "," + "'" + encoder + "'";
                                        ...
                                        try {
                                            ...
                                            LOG.info("id:{} update cv_dssm flag:{}", id, flag);
                                            if (flag != 0) {
                                        ...
                                        LOG.info("id:{} cv_dssm result is empty", id);
